...
public class JavaMonitor extends JavaMonitorQueuedSynchronizer {
    ...
    protected void setJfrOwner() {
        ...
        latestJfrTid = JavaThreads.getThreadId(currentThread);
        if (JavaThreads.isVirtual(currentThread)) {
            latestJfrVThreadName = currentThread.getName();
            latestJfrVThreadEpochId = JfrThreadRepository.getVThreadEpochId(currentThread);
        } else {
